...
class phpMyEdit
{
    ...
    function nav_bootstrap_links() { return stristr($this->navigation, 'Q'); }
	...
	{
		if (!in_array($this->fds[$k], $this->mass_save) && $this->fdd[$this->fds[$k]]['save']) $this->mass_save[] = $this->fds[$k];
		if (is_numeric($k)) {
			$k = $this->fds[$k];
		}
		$options = @$this->fdd[$k]['options'];
	...
	function sql_connect() /* {{{ */
	{
		$this->dbh = @ini_get('allow_persistent')
			? @mysqli_connect('p:'.$this->hn, $this->un, $this->pw, $this->dbh)
			: @mysqli_connect($this->hn, $this->un, $this->pw, $this->dbh);
	} /* }}} */
	...
	function myquery($qry, $line = 0, $debug = 0) /* {{{ */
	{
		...
		if ($debug_query || $debug) {
			$line = intval($line);
			echo '<h4>MySQL query at line ',$line,'</h4>',htmlspecialchars($qry),"\n";
		}
		if (isset($this->db)) {
		...
		if (! $ret) {
			...
			echo htmlspecialchars(mysqli_error($this->dbh)),"\n";
		}
		return $ret;
	} /* }}} */
	...
	function make_language_labels($language) /* {{{ */
	{
		...
		$file = $this->dir['lang'].$language.'.inc';
	...
	function form_begin() /* {{{ */
	{
		...
		if (isset($_POST)){
			foreach ($_POST as $k => $v) {
				if (preg_match('/^'.$this->cgi['prefix']['data'].$this->mass_save[0].'_([0-9]*)$/', $k, $matches)) $this->mass_save($matches[1], $this->key, $this->tb, $this->mass_save);
			}
		}
...
{';
	...
	document.getElementById('.$this->js['prefix'].'cur_tab+"_up_link").className = "','btn btn-block btn-info ',$css_class_name1,'";
	document.getElementById(tab_name+"_up_label").className = "',$css_class_name2,'";
	document.getElementById(tab_name+"_up_link").className = "','btn btn-block btn-info disabled ',$css_class_name2,'";';
				}
				if ($this->nav_down()) {
					echo '
	document.getElementById('.$this->js['prefix'].'cur_tab+"_down_label").className = "',$css_class_name1,'";
	document.getElementById('.$this->js['prefix'].'cur_tab+"_down_link").className = "','btn btn-block btn-info ',$css_class_name1,'";
	document.getElementById(tab_name+"_down_label").className = "',$css_class_name2,'";
	document.getElementById(tab_name+"_down_link").className = "','btn btn-block btn-info disabled ',$css_class_name2,'";';
				}
				echo '
...
function '.$this->js['prefix'].'filter_handler(theForm, theEvent)
{
	...
	function display_tab_labels($position) /* {{{ */
	{
		...
		for ($i = ($this->tabs[0] == 'PMEtab0' ? 1 : 0); $i < count($this->tabs); $i++) {
			$css_class_name = $this->getCSSclass($i != $this->cur_tab ? 'tab' : 'tab-selected disabled', $position);
			echo '<td class="',$css_class_name,'" id="'.$this->dhtml['prefix'].'tab',$i,'_',$position,'_label">';
			echo '<a class="btn btn-block btn-info ',$css_class_name,'" id="'.$this->dhtml['prefix'].'tab',$i,'_',$position,'_link';
	...
	function display_add_record() /* {{{ */
	{
		for ($tab = 0, $k = 0; $k < $this->num_fds; $k++) {
            ...
            if(isset($this->fdd[$k]['addcss']) && $this->fdd[$k]['addcss'] != '') $css_class_name .= ' '.$this->fdd[$k]['addcss'];
			...
			echo $this->getColAttributes($k),"><div id=\"",$this->cgi['prefix']['data'].$this->fds[$k],"_group\" class=\"control-group\">\n";
			if ($this->col_has_values($k)) {
				...
				echo '<textarea class="',$css_class_name,'" name="',$this->cgi['prefix']['data'].$this->fds[$k],'" id="',$this->cgi['prefix']['data'].$this->fds[$k],'"';
            ...
            } elseif (isset($this->fdd[$k]['add_display']) && $this->fdd[$k]['add_display'] != '') {
                ...
                if(isset($this->fdd[$k]['file']) && is_array($this->fdd[$k]['file'])) {
                    ...
                    echo '<input class="form-control ',$css_class_name,'" ';
			...
			if (isset($this->fdd[$k]['add_code']) && $this->fdd[$k]['add_code'] != '') echo $this->fdd[$k]['add_code'];
			if (isset($this->fdd[$k]['modal_code']) && $this->fdd[$k]['modal_code'] != '') $this->modal_code .= $this->fdd[$k]['modal_code'];
			echo '<div id="',$this->cgi['prefix']['data'].$this->fds[$k],'_alert"><span id="',$this->cgi['prefix']['data'].$this->fds[$k],'_help" class="help-inline"></span></div></div></td>',"\n";
	...
	function display_copy_change_delete_record() /* {{{ */
	{
		...
		for ($tab = 0, $k = 0; $k < $this->num_fds; $k++) {
			...
			if ($this->copy_operation() || $this->change_operation()) {
				if ($this->hidden($k)) {
					if ($k != $this->key_num) {
                        if (!isset($this->fdd[$k]['fdefault'])){
                            echo $this->htmlHiddenData($this->fds[$k], $row["qf$k"]);
                        } else {
                            if ($this->copy_operation()) {
								echo $this->htmlHiddenData($this->fds[$k], $this->fdd[$k]['fdefault']);
							} else {
								echo $this->htmlHiddenData($this->fds[$k], $row["qf$k"]);
							}
                        }
	 ...
	 * Mass save fields
	 */
	function saveDisplay($k, $row)
	{
		return $this->display_change_field($row, $k, true);
	}

	function display_change_field($row, $k, $massave=false) /* {{{ */
	{
		$name = ($massave) ? '_'.$row['qf'.$this->key_num] : '';
		$css_postfix    = @$this->fdd[$k]['css']['postfix'];
		$css_class_name = $this->getCSSclass('input', null, true, $css_postfix);
        if(isset($this->fdd[$k]['addcss']) && $this->fdd[$k]['addcss'] != '') $css_class_name .= ' '.$this->fdd[$k]['addcss'];
		$escape         = isset($this->fdd[$k]['escape']) ? $this->fdd[$k]['escape'] : true;
		if (!$massave) echo '<td class="',$this->getCSSclass('value', null, true, $css_postfix),'"',$this->getColAttributes($k),">";
		echo "<div id=\"",$this->cgi['prefix']['data'].$this->fds[$k],"_group\" class=\"control-group\">\n";
		if ($this->col_has_values($k)) {
			$vals       = $this->set_values($k);
			$multiple   = $this->col_has_multiple($k);
			$readonly   = $this->readonly($k);
			...
			//$escape     = true;
			if ($this->col_has_checkboxes($k) || $this->col_has_radio_buttons($k)) {
				if (isset($this->fdd[$k]['fdefault'])) $row["qf$k"] = $this->fdd[$k]['fdefault'];
                echo $this->htmlRadioCheck($this->cgi['prefix']['data'].$this->fds[$k].$name,
				...
				if (isset($this->fdd[$k]['fdefault']))
				{
					echo $this->htmlSelect($this->cgi['prefix']['data'].$this->fds[$k].$name,
						$css_class_name, $vals, $this->fdd[$k]['fdefault'], $multiple, $readonly,
						$strip_tags, $escape);
				} else {
                    echo $this->htmlSelect($this->cgi['prefix']['data'].$this->fds[$k].$name,
						$css_class_name, $vals, $row["qf$k"], $multiple, $readonly,
			...
			echo '<textarea class="',$css_class_name,' form-control" name="',$this->cgi['prefix']['data'].$this->fds[$k].$name,'" id="',$this->cgi['prefix']['data'].$this->fds[$k].$name,'"';
			echo ($this->readonly($k) ? ' disabled' : '');
			if (intval($this->fdd[$k]['textarea']['rows']) > 0) {
				echo ' rows="',$this->fdd[$k]['textarea']['rows'],'"';
			}
			...
			echo include($this->fdd[$k]['php']);
		} elseif (isset($this->fdd[$k]['change_display']) && $this->fdd[$k]['change_display'] != '') {
			$change_display = $this->substituteVars($this->fdd[$k]['change_display'], array('key'=>$row['qf'.$this->key_num]));
            echo $change_display;
		} else {
            if(isset($this->fdd[$k]['file']) && is_array($this->fdd[$k]['file'])) {
                ...
                echo '<input class="',$css_class_name,' form-control" type="text"';
                echo ($this->readonly($k) ? ' disabled' : '');
                echo ' name="',$this->cgi['prefix']['data'].$this->fds[$k].$name,'" id="',$this->cgi['prefix']['data'].$this->fds[$k].$name,'" value="';
                if (!isset($this->fdd[$k]['fdefault'])){
                    if($escape) echo htmlspecialchars($row["qf$k"]);
                    else echo $row["qf$k"];
                } else {
                    echo $this->fdd[$k]['fdefault'];
                }
                echo '"',$len_props,' />',"\n";
            }
		}
		echo '<div id="',$this->cgi['prefix']['data'].$this->fds[$k],'_alert"><span id="',$this->cgi['prefix']['data'].$this->fds[$k],'_help" class="help-inline"></span></div></div>';
		if (isset($this->fdd[$k]['add_code']) && $this->fdd[$k]['add_code'] != '') echo $this->fdd[$k]['add_code'];
		if (isset($this->fdd[$k]['modal_code']) && $this->fdd[$k]['modal_code'] != '') $this->modal_code .= $this->fdd[$k]['modal_code'];
		if (!$massave) echo '</td>',"\n";
	} /* }}} */
	...
	function display_password_field($row, $k) /* {{{ */
	{
		...
		echo '<input class="',$this->getCSSclass('value', null, true, $css_postfix),' form-control" type="password"';
		echo ($this->readonly($k) ? ' disabled' : '');
	...
	function cellDisplay($k, $row, $css) /* {{{ */
	{
		...
		if (strlen($value) <= 0 && !isset($this->fdd[$k]['file'])) {
			return '&nbsp;';
		}
		if ($escape) {
     ...
     * @param	bool
     */
    function check_file($path, $id, $unlink = false) {
        $int_dir = $this->get_folder($id);
        $path = '.'.$path.$int_dir.'/';
        $check = false;
     ...
     * @param	int
     */
    function file_upload($filevals, $id, $iid) {
        $int_dir = $this->get_folder($id);
        foreach ($filevals as $key => $value){
            if (@$_FILES[$key]['tmp_name'] != '' && @$_FILES[$key]['tmp_name'][0] != '')
            {
                $opath = FCPATH.substr($value['url'], 1);
            	$path = $opath.$int_dir.'/';
                if(!is_dir($opath)) mkdir($opath, 0755, true);
                if(!is_dir($path)) mkdir($path, 0755, true);

                $trashvals = array($key => $value);
                if($iid) $this->file_delete($trashvals, $id, $iid);

                if (!is_array($_FILES[$key]['name'])) {
                    $pieces = explode(".", $_FILES[$key]['name']);
                ...
                if (is_array($_FILES[$key]['name'])) {
                    ...
                    if(!is_dir($id_path)) mkdir($id_path, 0755, true);
                    foreach ($_FILES[$key]['name'] as $file_key => $file_value){
                        $pieces = explode(".", $_FILES[$key]['name'][$file_key]);
                        $extension = strtolower($pieces[count($pieces)-1]);
                        $file_path =  $id_path . $i . '.' . $extension;
                        move_uploaded_file($_FILES[$key]['tmp_name'][$file_key], $file_path);
                        $i++;
                    }
                }
            }
    ...
    /**
     * Upload file for changing
     *
     * @param	array
     * @param	int
     * @param	int
     */
    function file_ch_upload($filevals, $id, $iid)
    {
        foreach ($filevals as $key => $value)
        {
            if (@$_FILES[$key]['tmp_name'] != '' && @$_FILES[$key]['tmp_name'][0] != '')
            {
                ...
                $this->file_upload($filevals2, $id, $iid);
            }
        }
    }
     ...
     * @param	int
     */
    function file_delete($filevals = array(), $id, $iid) {
        $int_dir = $this->get_folder($id);
        foreach ($filevals as $key => $value){
            $path = FCPATH.substr($value['url'], 1).$int_dir.'/';
            if ($value['multiple']) {
                $trash_path = $path.'trash/'.$iid.'/'.$id.'/';
                $id_path = $path.$id.'/';
                if(!is_dir($trash_path)) mkdir($trash_path, 0755, true);
                if (is_dir($id_path)) {
                    if ($handle = opendir($id_path)) {
                        while (false !== ($file = readdir($handle))) {
                            if (preg_match ("/^[0-9a-zA-z_]*\.([[:alnum:]])*$/", $file)) {
                                if(!is_dir($id_path.$file)) copy($id_path.$file, $trash_path.$file);
                                if(!is_dir($id_path.$file)) unlink ($id_path.$file);
                            }
                        }
                        rmdir($id_path);
                    }
                ...
                $trash_path = $path.'trash/'.$iid.'/';
                if(!is_dir($trash_path)) mkdir($trash_path, 0755, true);
                if ($handle = opendir($path)) {
                    while (false !== ($file = readdir($handle))) {
                        if (preg_match ("/^".intval($id)."\.([[:alnum:]])*$/", $file) || preg_match ("/^".intval($id)."_([[:alnum:]])*\.([[:alnum:]])*$/", $file)) {
                            copy($path.$file, $trash_path.$file);
                            unlink ($path.$file);
                        }
                    }
                }
	...
	function htmlSubmit($name, $label, $css_class_name, $js_validation = true, $disabled = false, $js = NULL) /* {{{ */
	{
        ...
        if ($css_class_name == 'pme-query') $css_class_name .= ' btn btn-info';
		if ($css_class_name == 'pme-search' || $css_class_name == 'pme-hide') $css_class_name .= ' btn btn-default';
		if ($css_class_name == 'pme-clear') $css_class_name .= ' btn btn-warning';
		if ($css_class_name == 'pme-cancel') $css_class_name .= ' btn btn-default';
		if ($css_class_name == 'pme-add' || $css_class_name == 'pme-more' || $css_class_name == 'pme-change') $css_class_name .= ' btn btn-primary';
		if ($css_class_name == 'pme-save' && $css_class_name != 'PME_sys_savedelete') $css_class_name .= ' btn btn-success';
		if ($css_class_name == 'PME_sys_savedelete') $css_class_name .= ' btn btn-danger';
	...
	function htmlHidden($name, $value) /* {{{ */
	{
		return '<input type="hidden" name="'.htmlspecialchars($name)
			.'" id="'.htmlspecialchars($name).'" value="'.htmlspecialchars($value).'" />'."\n";
	} /* }}} */
	...
	{
        $ret = '<select class="'.htmlspecialchars($css).' form-control" id="'.htmlspecialchars($name).'" name="'.htmlspecialchars($name);
		if ($multiple) {
			...
			/* booleans: */ $multiple = false, $readonly = false, $strip_tags = false, $escape = true, $js = NULL)
	{
		...
		$found = false;
		foreach ($kv_array as $key => $value) {
			$ret .= ($multiple ? '<div class="checkbox"><label>' : '<div class="radio"><label>').'<input type="'.($multiple ? 'checkbox' : 'radio').'" name="';
	...
	function display_button($name, $position = 'up') /* {{{ */
	{
		...
		if ($name == 'goto_combo') {
			...
			return $this->htmlSelect($this->cgi['prefix']['sys'].ltrim($disabledgoto).'navfm'.$position,
					$this->getCSSclass('goto', $position).' input-inline', $kv_array, (string)$this->fm, false, $disabledgoto,
		...
		if (in_array($name, array('first','prev','next','last','<<','<','>','>>'))) {
			...
			return $this->htmlSubmit('navop', ucfirst($name),
					$this->getCSSclass($name2, $position).' btn btn-default', false, $$disabled_var ? $disabled : 0);
		}
	...
	function do_add_record() /* {{{ */
	{
		...
		for ($k = 0; $k < $this->num_fds; $k++) {
            ...
            if(is_array(@$this->fdd[$k]['file'])) $filevals[$this->fds[$k]] = $this->fdd[$k]['file'];
		}
		...
		if ($this->logtable) {
			$query = sprintf('INSERT INTO %s'
                    .' (id, pid, description, updated, user, host, operation, tab, rowkey, col, files, oldval, newval)'
					.' VALUES'
                    .' ("", "", "%s", NOW(), "%s", "%s", "insert", "%s", "%s", "", "%s", "", "%s")',
					$this->logtable,
                    addslashes($this->logtable_title.' "'.$newvals[$this->logtable_field].'"'),
                    addslashes($this->user), addslashes($this->get_server_var('REMOTE_ADDR')),
                    addslashes($this->tb), addslashes($key_col_val),
                    (count($filevals)) ? addslashes(serialize($filevals)) : '',
                    addslashes(serialize($newvals)));
			$this->myquery($query, __LINE__);
            $ch_iid = $this->sql_insert_id();
		}
        // Upload files
        $this->file_upload($filevals, $this->rec, 0);
		// After trigger
	...
	function do_change_record() /* {{{ */
	{
			...
			foreach ($filevals as $key => $value){
	            if (@$_FILES[$key]['tmp_name'] != '' && @$_FILES[$key]['tmp_name'][0] != '')
	            {
	            	$qry = sprintf('INSERT INTO %s'
						...
						addslashes($this->user), addslashes($this->get_server_var('REMOTE_ADDR')),
                        addslashes($this->tb),
						addslashes($this->rec), $this->labels['Reload'],
                        (count($filevals)) ? addslashes(serialize($filevals)) : '',
						addslashes($oldvals[$key]), addslashes($newvals[$key]));
					$this->myquery($qry, __LINE__);
	                $ch_iid = $this->sql_insert_id();
        ...
        // Upload files
        $this->file_ch_upload($filevals, $this->rec, $ch_iid);
		// After trigger
	...
	function do_delete_record() /* {{{ */
	{
		...
		if ($this->logtable) {
			$query = sprintf('INSERT INTO %s'
					.' (id, pid, description, updated, user, host, operation, tab, rowkey, col, files, oldval, newval)'
					.' VALUES ("", "", "%s", NOW(), "%s", "%s", "delete", "%s", "%s", "", "%s", "%s", "")',
					$this->logtable,
                    addslashes($this->logtable_title.' "'.$oldvals[$this->logtable_field].'"'),
                    addslashes($this->user),
					addslashes($this->get_server_var('REMOTE_ADDR')), addslashes($this->tb),
					addslashes($this->rec), (count($filevals)) ? addslashes(serialize($filevals)) : '', addslashes(serialize($oldvals)));
			$this->myquery($query, __LINE__);
            $ch_iid = $this->sql_insert_id();
		}
        // Delete files
        $this->file_delete($filevals, $this->rec, $ch_iid);
		// After trigger
	...
	function backward_compatibility() /* {{{ */
	{
		foreach (array_keys($this->fdd) as $column) {
			...
			if (isset($this->fdd[$column]['options'])) {
                $this->fdd[$column]['input'] .= stristr($this->fdd[$column]['options'], 'H') ? 'H' : '';
                $this->fdd[$column]['input'] .= stristr($this->fdd[$column]['options'], 'W') ? 'W' : '';
                $this->fdd[$column]['input'] .= stristr($this->fdd[$column]['options'], 'R') ? 'R' : '';
			}
		}
	} /* }}} */
	...
	function mass_save($id, $id_name, $table_name, $names)
	{
		foreach ($names as $v) {
			$data[$v] = $_POST['PME_data_'.$v.'_'.$id];
		}
	...
	function move_row($id, $move)
	{
        $query = $this->CI->db->query("SELECT ".$this->ui_sort_field." FROM ".$this->tb." WHERE ".$this->key."=".$id);
        $row = $query->row_array();

        if($move == 'up')
		{
            $query_subst = $this->CI->db->query("SELECT ".$this->key.", ".$this->ui_sort_field." FROM ".$this->tb." WHERE ".$this->filters." AND ".$this->ui_sort_field."<".$row[$this->ui_sort_field]." ORDER BY ".$this->ui_sort_field." DESC LIMIT 1");
        }

		if($move == 'down')
		{
            $query_subst = $this->CI->db->query("SELECT ".$this->key.", ".$this->ui_sort_field." FROM ".$this->tb." WHERE ".$this->filters." AND ".$this->ui_sort_field.">".$row[$this->ui_sort_field]." ORDER BY ".$this->ui_sort_field." LIMIT 1");
        }

		if ($query_subst->num_rows() == 1)
		{
			$row_subst = $query_subst->row_array();
			$query_down_up = $this->CI->db->query("UPDATE ".$this->tb." SET ".$this->ui_sort_field."=".($row[$this->ui_sort_field])." WHERE ".$this->key."=".$row_subst[$this->key]);
			$query_up_down = $this->CI->db->query("UPDATE ".$this->tb." SET ".$this->ui_sort_field."=".($row_subst[$this->ui_sort_field])." WHERE ".$this->key."=".$id);
		}
	...
	function __construct($opts) /* {{{ */
	{
        ...
        if(isset($this->uri['id']) && isset($this->uri['move']) && preg_match("/^[a-z]*$/",$this->uri['move']) && preg_match("/^[0-9]*$/",$this->uri['id'])){
            $this->move_row($this->uri['id'], $this->uri['move']);
        }
		...
		if (! $this->nav_buttons() && ! $this->nav_text_links() && ! $this->nav_graphic_links() && ! $this->nav_bootstrap_links()) {
