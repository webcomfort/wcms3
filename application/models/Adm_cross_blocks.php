...
class Adm_cross_blocks extends CI_Model {
    ...
    function get_filters()
    {
        ...
        // Сессия
        if (!$this->session->userdata('cross_blocks_filter'))
        {
            $this->session->set_userdata(array('cross_blocks_filter' => current(array_keys($filter_values))));
        }

        if($this->input->post('cross_blocks_filter', true) && preg_int($this->input->post('cross_blocks_filter', true)))
        {
            $this->session->set_userdata(array('cross_blocks_filter' => $this->input->post('cross_blocks_filter', true)));
        }

        // Отображение
        $data = array(
            'filter_name'   => 'Выберите то, как вам удобнее будет редактировать блок',
            'filter_action' => '/'.$this->uri->segment(1).'/'.$this->uri->segment(2).'/',
            'filter_field'  => 'cross_blocks_filter',
            'filter_class'  => ' select2',
            'filter_active' => $this->session->userdata('cross_blocks_filter'),
	...
	function _get_crud_model ()
	{
        ...
        // Таблица
        $opts['tb'] = 'w_pages_cross_blocks';

        // Ключ
        $opts['key'] = 'cross_block_id';

        // Начальная и ручная(UI) сортировка
        $opts['sort_field'] = array('cross_block_label');
        ...
        // Фильтрация вывода
        $opts['filters'] = array (
            "cross_block_lang_id = '" . $this->session->userdata('w_alang') . "'"
        );
        ...
        $opts['logtable_title'] = 'Сквозной блок';
        $opts['logtable_field'] = 'cross_block_name';
        ...
        $opts['fdd']['cross_block_name'] = array(
            'name'          => 'Название блока',
            'options'       => 'LACPDV',
            'select'        => 'T',
            'maxlen'        => 65535,
        ...
        $opts['fdd']['cross_block_label'] = array(
            'name'          => 'Метка блока',
            'options'       => 'LACPDV',
            'select'        => 'T',
            'maxlen'        => 65535,
        ...
        $opts['fdd']['cross_block_content'] = array(
            ...
            'sort'          => true,
            'escape'        => false,
            'help'          => 'Введите требуемый текст.'
        );
        if ($this->session->userdata('cross_blocks_filter') == 1) $opts['fdd']['cross_block_content']['addcss'] = 'htmleditor';

		if($publish)
		{
			$opts['fdd']['cross_block_active'] = array(
				'name'          => 'Статус',
				'select'        => 'D',
				'options'       => 'LACPDV',
				'values2'       => array (
				...
				'save'          => true,
				'default'       => 0,
				'help'          => 'Неактивный блок не показывается на сайте'
			);
		}

        // ------------------------------------------------------------------------

        $opts['fdd']['cross_block_lang_id'] = array(
            'name'          => 'Язык',
            'select'        => 'T',
            'options'       => 'ACPH',
            'maxlen'        => 3,
